...
export default function Home(){
  ...
  return <section><div className="ux-page-heading"><div><h1>ค้นหารายวิชาที่ใช่</h1><p>อ่านประสบการณ์จากผู้เรียน เลือกปี เทอม และผู้สอนให้ตรงกัน</p></div><Link className="btn btn-ghost" to="/courses/new">+ สร้างรายวิชา</Link></div>
    <form onSubmit={e=>{e.preventDefault();change({search:draft});}}><label htmlFor="course-search" className="ux-search-label">ค้นหารหัส ชื่อวิชา อาจารย์ หรือแท็ก</label><div className="ux-search-box"><input id="course-search" type="search" value={draft} onChange={e=>setDraft(e.target.value)} placeholder="เช่น แคลคูลัส รหัสวิชา หรือชื่อผู้สอน" maxLength={300}/>{draft&&<button type="button" className="ux-search-clear" aria-label="ล้างคำค้น" onClick={()=>{setDraft('');change({search:''});}}>×</button>}</div></form>
