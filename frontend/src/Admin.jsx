...
const emptyCourse = { course_code: "", course_name: "", department: "", prerequisites: "", syllabus: "", teaching_format: "", workload: "", assessment: "", tags_text: "", instructor_names: [], curriculum_id: "", recommended_year: "1", recommended_semester: "1", requirement_type: "REQUIRED" };
...
export default function Admin() {
  ...
  function startEdit(course) {
    const mapping = course.curriculum_mappings?.[0];
    setEditingId(course.course_id);
    setCourseForm({ ...emptyCourse, ...course, tags_text: (course.tags || []).join(", "), instructor_names: course.instructors || [], curriculum_id: mapping?.curriculum_id ? String(mapping.curriculum_id) : "", recommended_year: String(mapping?.recommended_year || 1), recommended_semester: mapping?.recommended_semester || "1", requirement_type: mapping?.requirement_type || "REQUIRED" });
    setTab("courses"); window.scrollTo({ top: 0, behavior: "smooth" });
  }
  async function saveCourse(event) {
    event.preventDefault(); setBusyId("course-form");
    try {
      const { curriculum_id, recommended_year, recommended_semester, requirement_type, tags_text, instructor_names, ...base } = courseForm;
      const curriculum_mappings = curriculum_id ? [{ curriculum_id: Number(curriculum_id), recommended_year: Number(recommended_year), recommended_semester, requirement_type }] : [];
  ...
  return <section>
    ...
    {tab === "courses" && <>
      ...
      <form className="card" onSubmit={saveCourse}>
        ...
        <div className="row"><Select label="หลักสูตร (ไม่บังคับ)" value={courseForm.curriculum_id} onChange={setCourse("curriculum_id")}><option value="">ยังไม่ผูกหลักสูตร</option>{curriculums.map((c) => <option value={c.curriculum_id} key={c.curriculum_id}>{c.curriculum_name} ({c.academic_year})</option>)}</Select><Select label="ประเภทวิชา" value={courseForm.requirement_type} onChange={setCourse("requirement_type")}><option value="REQUIRED">วิชาบังคับ</option><option value="ELECTIVE">วิชาเลือก</option></Select></div>
        {courseForm.curriculum_id && <div className="row"><Field label="ชั้นปีแนะนำ" type="number" value={courseForm.recommended_year} onChange={setCourse("recommended_year")} required /><Field label="เทอมแนะนำ" value={courseForm.recommended_semester} onChange={setCourse("recommended_semester")} required /></div>}
